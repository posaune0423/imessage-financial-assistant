...
- multi-user 対応は `sender/chatId -> user -> wallet context` を `agent.generate()` の前に解決する
...
- iMessage sender 単位で user を解決する
- user が iMessage だけで wallet 作成・残高確認・署名・trade を完結できる
- user に primary wallet context を紐づける
...
- 既存の Mastra memory を user 単位の stable resource key で継続利用する
...
1. ユーザーが DM を送る
2. `sender` / `chatId` を messaging identity として user を解決または作成する
...
2. backend がその phone number を canonical identity として user を resolve/create する
...
6. backend は Turnkey JWT claims を検証し、user / sub-org と照合する

### Wallet / Crypto Questions

- 「自分の wallet」「残高」「ポジション」「open orders」「BTC の状況」などは local tool で解決する
- Hyperliquid account 参照は user の primary EVM address を使う
...
2. **Ownership-authenticated action**
   ...
   - backend は verification token / session JWT を検証し、user と sub-org を照合する
...
tool 自体は static に登録し、実行時に `requestContext.userId` をキーに DB と service layer から runtime を解決する。
...
- server は signer readiness と user / sub-org / delegated-user の対応を検証する
...
- `sqlite/schema.ts`
- `sqlite/client.ts`
- `sqlite/sqlite-user-repository.ts`
...
- どちらの mode でも backend で user / sub-org / policy 対応を検証する
...
- e2e: different senders get isolated user/resource keys and replies
...
- sender/chatId ごとに独立した user と Mastra memory resource が解決される
